            ...
            success: function(options) {
                    ...
                    options.categories.forEach(category => {
                        $categoryFilters.append(`
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="category${category.id}">${category.name} (${category.count || 0})</label>
                            </div>
                        `);
                    });
                }
                ...
                if (options.brands && Array.isArray(options.brands)) {
                    options.brands.forEach(brand => {
                        $brandFilters.append(`
                            <div class="form-check">
                                <input class="form-check-input brand-filter" type="checkbox" id="brand${brand.brand}" value="${brand.brand}">
                                <label class="form-check-label" for="brand${brand.brand}">${brand.brand} (${brand.count || 0})</label>
                            </div>
                        `);
                    });
                }
                ...
                if (options.sizes && Array.isArray(options.sizes)) {
                    options.sizes.forEach(size => {
                        $sizeFilters.append(`
                            <div class="form-check">
                                <input class="form-check-input size-filter" type="checkbox" id="size${size.size}" value="${size.size}">
                                <label class="form-check-label" for="size${size.size}">${size.size} (${size.count || 0})</label>
                            </div>
                        `);
                    });
                }
                ...
                if (options.colors && Array.isArray(options.colors)) {
                    options.colors.forEach(color => {
                        $colorFilters.append(`
                            <div class="form-check">
                                <input class="form-check-input color-filter" type="checkbox" id="color${color.color}" value="${color.color}">
                                <label class="form-check-label" for="color${color.color}">${color.color} (${color.count || 0})</label>
                            </div>
                        `);
                    });
                }
                ...
                if (options.occasions && Array.isArray(options.occasions)) {
                    options.occasions.forEach(occasion => {
                        $occasionFilters.append(`
                            <div class="form-check">
                                <input class="form-check-input occasion-filter" type="checkbox" id="occasion${occasion.occasion}" value="${occasion.occasion}">
                                <label class="form-check-label" for="occasion${occasion.occasion}">${occasion.occasion} (${occasion.count || 0})</label>
                            </div>
                        `);
                    });
                }
                ...
                if (filterType === 'discount') {
                    ...
                    const categoryName = options.categories.find(c => c.id == categoryId)?.name;
                    if (categoryName) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response && response.products) {
                ...
                if (filters.search) {
                    $('#productListingTitle').html(`Search Results for: <em>"${filters.search}"</em> (${response.pagination?.total_items || 0} items)`);
                } else {
                    $('#productListingTitle').text(`All Products (${response.pagination?.total_items || 0} items)`);
                }
            } else {
                showNoResults();
            }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.loggedIn) {
                $('#authLinks').hide();
                $('#userLinks').show();
                $('#userDropdown i').after('<span class="d-none d-lg-inline"> ' + response.user.name + '</span>');
            } else {
                $('#authLinks').show();
                $('#userLinks').hide();
            }
                    ...
                    <div class="card product-card h-100">
                        ...
                        <a href="product-detail.html?id=${product.id}">
                            <img id="${imageId}" 
                                 ...
                                 alt="${product.name}"
                                 data-original-src="assets/uploads/${product.image1}"
                            ...
                            <div class="col-md-6">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex gap-2 mb-2">
                                        ${product.size ? `<span class="badge bg-light text-dark">${product.size}</span>` : ''}
                                        ${product.color ? `<span class="badge bg-light text-dark">${product.color}</span>` : ''}
                                        ${product.brand ? `<span class="badge bg-light text-dark">${product.brand}</span>` : ''}
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
...
function tryAlternativeImage($img, alternatives, index) {
    if (index >= alternatives.length) {
        ...
        $img.attr('src', createPlaceholderDataURL($img.attr('alt') || 'Product Image'));
        return;
    }
