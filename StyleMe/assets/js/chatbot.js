class StyleMeChatbot {
    ...
    isProductSearchQuery(message) {
        const productKeywords = [
            'shirt', 'dress', 'pants', 'jeans', 'shoes', 'jacket', 'skirt', 'saree', 'kurta',
            'find', 'search', 'show', 'looking for', 'need', 'want to buy', 'recommend',
            'casual', 'formal', 'party', 'wedding', 'office', 'summer', 'winter',
            'red', 'blue', 'black', 'white', 'green', 'pink', 'yellow', 'purple',
    ...
    async handleProductSearch(message) {
        try {
            ...
            if (data.success && data.products && data.products.length > 0) {
                this.addBotMessage(`Great! I found ${data.products.length} items that match what you're looking for:`);
                ...
                this.addBotMessage("Would you like me to find something more specific? 🛍️", true);
            } else {
                this.addBotMessage(
    ...
    async handleGeneralQuery(message) {
        ...
        if (message.toLowerCase().includes('hello') || message.toLowerCase().includes('hi')) {
            response = "Hello! 👋 Welcome to StyleMe! I'm here to help you find the perfect outfit. What can I help you with today?";
        } else if (message.toLowerCase().includes('price') || message.toLowerCase().includes('cost')) {
            response = "Our prices range from affordable everyday wear to premium fashion pieces. You can filter by price range on our products page. What's your budget?";
        } else if (message.toLowerCase().includes('delivery') || message.toLowerCase().includes('shipping')) {
            response = "We offer fast delivery across Sri Lanka! 🚚\n\n• Colombo: 1-2 days\n• Other cities: 2-3 days\n• Free shipping on orders over Rs. 3,000\n\nWould you like to check delivery for a specific area?";
        } else if (message.toLowerCase().includes('size') || message.toLowerCase().includes('fit')) {
            response = "We have a comprehensive size guide! 📏\n\n• XS to XXL available\n• Size chart on each product\n• Easy returns if size doesn't fit\n\nNeed help with sizing for a specific item?";
        } else if (message.toLowerCase().includes('return') || message.toLowerCase().includes('exchange')) {
            response = "Our return policy is customer-friendly! ↩️\n\n• 7-day return window\n• Easy online returns\n• Free exchanges for different sizes\n• Full refund if not satisfied\n\nNeed help with a return?";
        } else {
            response = "I'm still learning! 🤖 For detailed questions, you can:\n\n• Contact our customer service\n• Check our FAQ section\n• Ask me about specific products\n\nIs there anything specific you're looking for today?";
        }
    ...
    addProductCard(product) {
        const imageUrl = product.image1 ? `assets/uploads/${product.image1}` : 'assets/images/placeholder-product.jpg';
        ...
        const productHTML = `
            <div class="chatbot-message bot">
                ...
                <div class="chatbot-message-content">
                    <div class="chatbot-product-card" onclick="window.open('product-detail.html?id=${product.id}', '_blank')">
                        <img src="${imageUrl}" alt="${product.name}" class="chatbot-product-image" 
                             onerror="this.src='assets/images/placeholder-product.jpg'">
                        <div class="chatbot-product-info">
                            <h6>${product.name}</h6>
                            <p>${product.category_name || 'Fashion'} ${product.brand ? '• ' + product.brand : ''}</p>
